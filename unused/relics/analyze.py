...
for term in ex_terms:
    ...
    #conjunctions
    conjs = ["atque","+que","ac","et","sed","at","aut","autem","tamen","si","nisi","tam","ita","sic","ut","num","ne","neque","enim","nam","dum","cum","quod"]
...
for col in range(len(final_list)):
    if parsing == "n": final_list[col][2] = ""
        ...
        final_list[col][2] = ["<input type='text' value='%s'>"%(final_list[col][2]),""][method%2]
...
if method == 0: from output0 import *
elif method == 1: from output1 import *
else: from output2 import *
format(final_list,report,parsing,errors)
os.remove("output%i.pyc"%(method))
#print report

if method == 0:
    f = open("../site/output.html","r")
    print f.read()
